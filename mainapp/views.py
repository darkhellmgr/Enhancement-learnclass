...
def takequiz(request, uid, quizid):
    ...
    if token:
        ...
        db.child("Results").child(uid).child(quizid).child(attempt_id).update({'total_questions': no_of_questions},
                                                                              token)


        gender = getuserid("Student", "gender", token)
        scn = getuserid("Student", "special_care_needed", token)
        date_str = getuserid("Student", "dob", token)
        ...
        data = {
            'sid':uid,
            'gender':gender,
            'special_care_needed':scn,
            'age':age,
            'quizid':quizid,
            'total_marks': no_of_questions,

        }

        attempt_id = db.child("Machine_Learning").push(data, token)
        ...
        if request.method == "POST" and request.is_ajax:
            ...
            if questype == "subjective":
                ...
                if correctans == r['answer']:
                    ...
                    db.child("Results").child(uid).child(quizid).child(attempt_id).update(
                        {'total_correctans': no_of_correctans + 1}, token)
                    db.child("Machine_Learning").update({'marks': no_of_correctans + 1}, token)
...
def time_taken(request, quizid, timetaken, attempt_id):
    ...
    if token:
        ...
        no_of_correctanswer = db.child("Results").child(uid).child(quizid).child(attempt_id).child(
            "total_correctans").get(token).val()
        print("coreect answer")
        print(no_of_correctanswer)
        total_questions = db.child("Results").child(uid).child(quizid).child(attempt_id).child("total_questions").get(
            token).val()
